//=============================================================================
// Brief   : Factory for Policy Retreiver implementations
 ...
 * @since 01-03-2011
 */
public final class PolicyRetreiverFactory extends ClassFactory {

    /**
     * Used to define a context.
     */
    private static final String CONTEXT = "PolicyRetreiverFactory";
    /**
     * Inside module folder, where modules.xml is, there should be a
     * policyretreiver folder to search for IPolicyRetreiver implementation.
     *<br/>
     ...
     */
    private static final String PATH = "policyretreiver";
    /**
    ...
    /**
     * Bad classes are classes that exist but are not IPolicyRetreiver
     * implementations.
     */
    private List<String> badClasses;
    /**
     * To avoid problems with libs being loaded by different applications,
     * all instances of PolicyRetreiverFactory will be single
     */
    private Map<String, IPolicyRetreiver> instances;

    public PolicyRetreiverFactory(Set<AvailableClass> allclasses, boolean web, ClassLoader parentClassLoader) throws InvalidPathToJarException, NotReadableException {
        classes = new HashMap<String, AvailableClass>();
        badClasses = new ArrayList<String>();
        instances = new HashMap<String, IPolicyRetreiver>();
            ...
            for (AvailableClass iclass : allclasses) {
                if (iclass.getLocation() != null) {
                    try {
                        ...
                        Logger.getLogger(PolicyRetreiverFactory.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            }
        ...
        for (AvailableClass iclass : allclasses) {
            try {
                ...
                //Object t = get(iclass.getClassName());//new PolicyRetreiver();
                ...
                //if (t instanceof IPolicyRetreiver) {
                if (IPolicyRetreiver.class.isAssignableFrom(t)) {
                    debug("is instance of IPolicyRetreiver");
                    classes.put(iclass.getClassName(), iclass);
                }

                debug("is instance of IPolicyRetreiver? " + (IPolicyRetreiver.class.isAssignableFrom(t)));
    ...
    @Override
    public IPolicyRetreiver get(String className) throws ClassNotFoundException, InstantiationException, IllegalAccessException {
        IPolicyRetreiver nclass;
        ...
        if (badClasses.contains(className)) {
            throw new InstantiationException("Invalid class: " + className + ". Its not a PolicyRetreiver class.");
        }
        ...
        nclass = (IPolicyRetreiver) jcl.loadClass(className);
    ...
    protected static void debug(String message) {
        if (debug) {
            debugStream.println(DEBUGCLASSNAME + "[PolicyRetreiverFactory] " + message);
        }
    }
}
